import 'package:gymbro/features/ai_chat/domain/aiml_api_service.dart';
...
class AimlChatMessages extends _$AimlChatMessages {
  ...
  final _systemPrompt = 'Ты персональный тренер по фитнесу, эксперт по питанию и тренировкам. Отвечай коротко и давай полезные советы.';
  ...
  Future<void> sendMessage(String text) async {
    final AimlApiService chatService = ref.read(aimlApiServiceProvider);
    ...
    try {
      ...
      final response = await chatService.sendMessage(text, systemPrompt: _systemPrompt);
  ...
  void clearChat() {
    ...
    // Возвращаем только начальное сообщение
    state = [
      types.TextMessage(
        author: _aiUser,
        createdAt: DateTime.now().millisecondsSinceEpoch,
